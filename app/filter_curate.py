...
def filter_top_n(data, n=20):
    ...
    return {"items": top_n_items, "count": len(top_n_items)}
...
def filter_vietnamese(data):
    ...
    items = data.get("items", [])
    vietnamese_items = [
        font for font in items if "vietnamese" in font.get("subsets", [])
    ]

    return {"items": vietnamese_items, "count": len(vietnamese_items)}
